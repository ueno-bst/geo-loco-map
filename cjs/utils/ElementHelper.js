...
function toPixel(value) {
    return (0, Types_1.isNumber)(value) ? value + "px" : "";
}
const EventRegex = {
    KeyboardEvent: /^key/,
...
class ElementHelper {
    ...
    constructor(node) {
        ...
        this.node = node;
        this.emitter = new EventEmitter_1.default(node);
    ...
    setPosition(value) {
        ...
        let top;
        let right;
        if (value instanceof LatLng_1.Rectangle) {
            left = value.left;
            right = value.right;
            top = value.top;
            bottom = value.bottom;
        }
        else if (value instanceof LatLng_1.Point) {
            left = value.x;
            top = value.y;
        }
        this.setStyles({
    ...
    first() {
        ...
        if (!(0, Types_1.isNull)(child) && isHtmlElement(child)) {
