...
export module GeoHash {

    const MAP32 = '0123456789bcdefghjkmnpqrstuvwxyz';

    function coordinateToBit(coordinate: number, min: number, max: number, length: number = 10): string {
        let bit: string = '';
    ...
    function bitToCoordinate(bit: string, min: number, max: number): number {
        let error = 0;
        let value = 0;
    ...
    export function encode(coordinate: ILatLng, length: number = 10) {
    ...
    export function decode(hash: string): LatLng {
        ...
        return new LatLng(
            bitToCoordinate(latitude_bits, -90, 90),
            bitToCoordinate(longitude_bits, -180, 180)
        );
    }
}